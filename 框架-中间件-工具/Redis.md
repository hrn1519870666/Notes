...
**定期删除** ： 每隔一段时间**随机抽取**一批设置了过期时间的 key 执行删除过期key操作（不是遍历所有的设置过期时间的 key，那样cpu 负载会很高）。
...
**解决思路 1：先删除缓存，再更新数据库。**如果数据库更新失败了，那么数据库中是旧数据，缓存中是空的，那么数据不会不一致。因为读的时候没有缓存，去读数据库中的旧数据，然后更新到缓存中。

**解决思路 2：延时双删。**依旧是先更新数据库，再删除缓存，不同的是，把这个删除的动作，在不久之后再执行一次，比如 5s 之后。
